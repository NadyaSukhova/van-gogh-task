...
const goodsList = document.getElementsByClassName('recommended-goods')[0];
let fullfilledGoodsList="";
for (let good of allGoods) {
    fullfilledGoodsList += `<article class='recommended-good'><img class='good-img' src='${good.img}' /><div class='good-content'><p class="good-description">${good.text}</p><div class="good-amount"><img class='amount-img' src='./images/check.svg'/ style='background-color:${good.length > 0 ? '#4FCF36' : '#EE753E'}'>${good.length > 0 ? 'В наличии '+String(good.length)+'м.'  : 'Под заказ'}</div><button class='good-more'>Подробнее</button></div></article>`;
}
